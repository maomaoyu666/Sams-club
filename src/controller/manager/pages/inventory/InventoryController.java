...
public class InventoryController {
        ...
        Callback<TableColumn<StoreInventory, Void>, TableCell<StoreInventory, Void>> cellFactory = new Callback<TableColumn<StoreInventory, Void>, TableCell<StoreInventory, Void>>() {
            ...
            public TableCell<StoreInventory, Void> call(final TableColumn<StoreInventory, Void> param) {
                return new TableCell<StoreInventory, Void>() {
                    ...
                    private final Button auditButton = new Button("Request");

                    {
                        auditButton.getStyleClass().add("button");
                        auditButton.getStyleClass().add("xs");
                        auditButton.getStyleClass().add("info");
                        auditButton.setOnAction((ActionEvent event) -> {
                            StoreInventory inventory = getTableView().getItems().get(getIndex());

                            Alert alert = new Alert(Alert.AlertType.INFORMATION);
                            alert.setHeaderText("Are you sure that you want to Request ?");
                            alert.setTitle("Request " + inventory.getItemName() + " ?");
                            ...
                            if (deleteConfirmation.isPresent() && deleteConfirmation.get() == ButtonType.OK) {
                                Datasource.getInstance().auditInventory(inventory.getStoreInventoryId());
                            }
                        });
                    }
...

                    {
                        ...
                        buttonsPane.getChildren().add(auditButton);
                    }
    ...
    private void btnAddInventoryOnClick() {
        ...
        try {
            fxmlLoader.load(getClass().getResource("/view/manager/pages/inventory/add-inventory.fxml").openStream());
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void btnEditProduct(long inventoryId) {
        FXMLLoader fxmlLoader = new FXMLLoader();
        try {
            fxmlLoader.load(getClass().getResource("/view/manager/pages/inventory/edit-inventory.fxml").openStream());
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void btnViewStore(long inventoryId) {
        FXMLLoader fxmlLoader = new FXMLLoader();
        try {
            fxmlLoader.load(getClass().getResource("/view/manager/pages/inventory/view-inventory.fxml").openStream());
        } catch (IOException e) {
            e.printStackTrace();
        }
